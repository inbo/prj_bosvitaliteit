...
De data uit de andere meetpunten wordt ook door ICP forests gebruikt in hun detailbespreking en modelbouw (\citep{icpforesttechreport}).
...
Er wordt jaarlijks een gedetailleerd INBO rapport gemaakt die de data bespreekt, zowel over de bevindingen van het laatste jaar, als vergelijkingen tussen de laatste jaren en een trendanalyse (\citep{bosvitaliteit2022}).
De laatste jaren zijn ook op een gelijkaardige manier specifiek de essen opgevolgd in een ander INBO project. De toestand van de es wordt ook in het bosvitaliteitsrapport meegenomen.
...
De resultaten van ICP forests worden bezorgd aan de Europese commissie (\citep{icpforesttechreport}).
...
De proefvlakken worden al heel lang opgevolgd, wat een mooie tijdreeks oplevert, maar intussen is de tijdsreeks al zo lang dat ook de leeftijd van de bomen een rol begint te spelen in de vitaliteit van de proefvlakken, wat eveneens niet meer representatief is voor de volledige bosgemeenschap.
...
Op zich is dit geen probleem, en is eigen aan iedere meting, dit is dus enkel een extra mogelijk meetfout die in het model in rekening kan gebracht worden.
...
Voor 2012 werd vanuit het gekozen gridpunt 4 clusters van 6 bomen gekozen binnen een vooropgestelde afstand van het gridpunt, waarbij de 4 clusters elk overeenkomen met 1 windrichting. Een boom die sterft of gekapt wordt tussen de verschillende meetperiodes wordt vervangen door een boom in de buurt.
...
De vorm van de proefvlakken en welke bomen gekozen worden verschilt van land per land, maar er is wel een basishandleiding van ICP forests waarop landen zich moeten baseren, maar waarbij lichte variaties toegelaten worden.
...
Ieder jaar worden de nieuwe data nog toegevoegd aan de dataset, maar ook niet veel meer dan dat. Zo zijn de nieuwste proefvlakken die bemonsterd worden nog niet toegevoegd aan de databank. Dit zal binnenkort aangepakt worden.
...
De Vlaamse overheid geeft een voorkeur naar de Creative Commons Unrestricted  (CC0) licentie (zie https://www.vlaanderen.be/inbo/en-gb/open-data-policy/). Er wordt van de ethiek van de wetenschapper verwacht dat verwezen wordt naar de herkomst van de data, maar juridisch kan dit niet verplicht worden.
...
Doordat de tijdreeks reeds heel lang is, is het moeilijk uit te gaan van een vaste trend over de hele periode, dus zal beperkt worden tot analyses over een rollend 12 jarig tijdvenster.

Daarnaast wordt ook een mixed effect model gebruikt om de datatrends te analyseren. Hoewel het bladverlies binnen een range van 0% tot 100% ligt, liggen de meeste waarden eerder in het midden van het bereik en is een normale benadering niet onlogisch. Het voorkeursmodel is een random slope per proefvlak en een random intercept per boom met autocorrelatie voor de metingen van dezelfde boom over de tijd, maar voor de powerberekeningen zullen we dit vereenvoudigen.
...
Vroeger is er eens een verklarend model gemaakt om de oorzaken van patronen in het blad- of naaldverlies te achterhalen \citep{bosvitaliteitmodelmatig}. Misschien is het een idee om dit periodiek eens te herhalen, maar voorlopig is daar nog niets voor gepland.


## Fase IV: Ontwerp rapportering en communicatie

- Het hoofdrapport \citep{bosvitaliteit2022} is een jaarlijks INBO-rapport
...
- Jaarlijks wordt ook gerapporteerd naar ICP forests
    ...
    - rapportage van alle proefvlakken voor ICP-forests jaarlijkse statistische analyse   \citep{icpforesttechreport} over de bosvitaliteit in Europa
...
- Enkel basisinformatie: de kroonbeoordeling en symptoombeschrijving
    ...
    - deze extra invloedsfactoren zijn enkel in 5 level-2 meetnetlocaties ter beschikking
    - deze zwakte is hetzelfde in de volledige Europese context
- mogelijks modernere manieren om de gezondheidstoestand op te volgen
    - bijvoorbeeld remote sensing, maar dat is vanuit een ander standpunt: daar wordt van boven naar onder gekeken in plaats van vanonder naar boven
- kroonbeoordeling is intens werk waar iedere boom toch enkele minuten bekeken moet worden
    - de methode is goed en er is geen methode aan de horizon te zien die beter zou kunnen presteren
- Representativiteit bij keuze van proefvlakken
...
- De proefvlakken worden ouder door de lange duur van het meetnet
    ...
    - een verslechterende trend kan door ouderdom komen in plaats van algemene vitaliteit in Vlaanderen, al moet gezegd worden dat de bomen in het meetnet zeker nog niet op een leeftijd hebben die problematisch is voor de gezondheid 
...
- Perspectieven voor verbanden tussen vitaliteit en beheervormen, afstand tussen bomen, enzovoort als huidige dendrometrische gegevens worden toegevoegd aan Fieldmap. De laast beschikbare dendrometische gegevens dateren uit Fieldmap in 2009-2010. Deze gegevens zitten ook niet in de database van bosvitaliteit.
