...
```{r trendperjaar, fig.cap= "trend per jaar (zwart geaggregeerd per plot, groen gewoon naïef over alle bomen heen zonder rekening te houden met plot)}
...
```{r evolutieperplot, fig.cap = "Evolutie bladverlies geaggregeerd per plot. Groene lijn is gemiddeld bladverlies over de dataset"}
df_plots <- df_trees %>% 
...
ggplot(df_plots, aes(x = Jaar, y = nnv, color = PlotNr)) + 
  geom_line() + 
...
```{r modeltest, eval = TRUE, echo = TRUE, cache = TRUE ,warning=FALSE}
library(nlme)
...
plotdata_full <- data.frame(
  ...
  ggplot(plotdata_full, aes(x = fit, y = resid, color = factor(Jaar))) + 
  geom_point(pch = 1) + labs(x = "Gefitte waarden", 
                             y = "Genormaliseerde residu's", 
                             color = "Jaar")
  ggplot(plotdata_full, aes(x = resid)) + geom_histogram()
...
plotdata_red <- data.frame(
  ...
  ggplot(plotdata_red, aes(x = fit, y = resid, color = factor(Jaar))) + 
  geom_point(pch = 1) + labs(x = "Gefitte waarden", 
                             y = "Genormaliseerde residu's", 
                             color = "Jaar")
  ggplot(plotdata_red, aes(x = resid)) + geom_histogram()
...
De schattingen tussen het ingewikkelde en vereenvoudigde model zijn compatibel met elkaar, en ook de standaardfout op de schatting van jaar is vergelijkbaar (ergens een 10% verschil), dus ook de power tussen beide modellen is hierdoor vergelijkbaar, waardoor het eenvoudigere model gebruikt kan worden.
