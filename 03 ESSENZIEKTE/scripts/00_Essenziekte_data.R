...
dfEsMeting %>%
  summarise(aantal_bomen = n(), .by = c(ProefvlakNummer, Jaar)) %>%
...
bm_prv_jr <- dfEsMeting %>%
  ...
  pivot_wider(id_cols = ProefvlakNummer, names_from = Jaar, values_from = aantal_bomen, values_fill = 0)
write_csv2(bm_prv_jr, file = paste0(out, "aantal_bomen_proefvlak_jaar.csv"))
...
dfEsSymptoomOorzAlle <-
  ...
  dplyr::inner_join(select(
    dfEsSymptoomOrigAlle, ProefvlakID, ProefvlakNummer,
    Jaar, BoomID, MetingID, MetingSymptoomID, GS
  ), by = "MetingSymptoomID") %>%
...
ggplot(
  dfEsMeting %>% group_by(ProefvlakNummer, Jaar) %>% summarize(gem_bladverlies = mean(Bladverlies)),
...
ggplot(
  dfEsMeting %>% filter(Jaar %in% es_jaren, gestorven == FALSE) %>% group_by(ProefvlakNummer, Jaar) %>% summarize(gem_bladverlies = mean(Bladverlies)),
  aes(x = Jaar, y = gem_bladverlies)
) +
  facet_wrap(~ProefvlakNummer) +
  geom_point() +
