...
dfEsMeting %>% 
  ...
  left_join(dfEsMeting %>% 
              ...
              mutate(probleem = jaren_aanwezig < max(jaren_aanwezig)), by = c("ProefvlakNummer", "BoomID")) %>%
...
dfEsMeting %>% 
  filter(Jaar == es_lastyear) %>% 
  group_by(ProefvlakID, Drainagegroepering, Waterbeschikbaarheid, `Textuur-gegroepeerd`) %>% 
...
(p <- ggplot(dfBladverliesklasse, aes(x = Jaar, y = percentage, fill = fct_rev(BladverliesklasseEur))) + 
...
(p <- ggplot(dfBeschadigd, aes(x = Jaar, y = pct_beschadigd, color = Leeftijd)) + 
...
(p <- ggplot(es_gem_lft , aes(x = Jaar, y = gem_BV, color = Leeftijd)) + 
  ...
  geom_errorbar(data = es_gem, aes(ymin = lcl_BV, ymax = ucl_BV), width = 0.2, size = 1) + 
...
(p <- ggplot(dfEsBV, aes(x = factor(Jaar), y = gem_BV, group = factor(ProefvlakNummer), 
                         color = Beschadigd, shape = Beschadigd)) + 
  geom_point() + geom_path(aes(group = ProefvlakNummer)) + scale_color_discrete("") + 
...
dfEsBesch <- 
  dfEsBV %>% 
  mutate(schadeklasse = cut(gem_BV, c(0,10,25,40,100), include.lowest = TRUE)) %>% 
